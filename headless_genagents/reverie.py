...
class ReverieServer: 
  def __init__(self, 
    ...
    create_folder_if_not_there(f"{fs_temp_storage}/curr_sim_code.json")
    with open(f"{fs_temp_storage}/curr_sim_code.json", "w") as outfile: 
      outfile.write(json.dumps(curr_sim_code, indent=2))
    
    curr_step = dict()
    curr_step["step"] = self.step
    create_folder_if_not_there(f"{fs_temp_storage}/curr_step.json")
    with open(f"{fs_temp_storage}/curr_step.json", "w") as outfile: 
      outfile.write(json.dumps(curr_step, indent=2))


  def write_headless_environment(self, movements):
    """
    Write the next environment file directly from backend movement results.

    In the original browser-coupled loop, the frontend reads movement/<step>.json,
    animates one tile, then writes environment/<step + 1>.json. Headless mode
    keeps the same backend contract by writing that next environment file here.
    """
    sim_folder = f"{fs_storage}/{self.sim_code}"
    next_env = dict()

    for persona_name, movement in movements["persona"].items():
      next_tile = movement["movement"]
    ...
    create_folder_if_not_there(next_env_file)
    ...
    while (True): 
      ...
      if check_if_file_exists(curr_env_file):
        ...
        if env_retrieved: 
          ...
          self.write_headless_environment(movements)
